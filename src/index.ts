...
export default class VitepressDocProcessor extends WProcessor {
    async build(content: Buffer | "dir"): Promise<ProcessorOutputRaw> {
        ...
        while (dirTocPath.length > 1) {
            ...
            dirTocProc = this.files({ include: tryPath, absolute: true }).values().next()?.value?.procs({ include: "dir-toc" }).get("dir-toc")?.values().next().value as unknown as FileNamedProcOne
